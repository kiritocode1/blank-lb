 ...
 */
export const EndpointUrl = Schema.String.pipe(Schema.pattern(/^https?:\/\/.+/), Schema.brand("EndpointUrl"));
/**
 * Schema for health check path (must start with /)
 */
export const HealthCheckPath = Schema.String.pipe(Schema.pattern(/^\/.*/), Schema.brand("HealthCheckPath"));
/**
 * An Endpoint represents a backend URL that can receive load-balanced traffic.
 */
export class Endpoint extends Schema.Class("Endpoint")({
    /**
     ...
     */
    buildTargetUrl(pathname, search) {
 ...
 */
export const endpoint = (url, options) => new Endpoint({
    url,
